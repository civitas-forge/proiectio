...
/// destination.
///
/// A plan is complete: apply takes only the plan, never the desired tree or
/// the disk, so each action carries what executing it needs — the [`Entry`]
/// to write, and for destructive actions the recorded hash to re-check
/// before touching the disk. `BTreeMap` keeps plans sorted, diffable, and
/// deterministic; apply derives execution order from it (parents before
/// children, removals in reverse).
///
/// An empty desired tree plans a removal of everything the owner holds.
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize)]
pub enum Action {
    /// Create a path that is neither recorded nor on disk.
    Write {
        /// What to write.
        entry: Entry,
    },
    /// Replace a recorded path whose disk bytes still match the recorded
    /// hash but whose desired content changed. Apply re-hashes the target
    /// first and refuses if the disk changed since the plan.
    Overwrite {
        /// What to write.
        entry: Entry,
        /// The hash the manifest records; the target must still match it at
        /// apply time.
    ...
    /// survives.
    Skip,
    /// Remove an orphan — recorded under this owner but absent from the
    /// desired tree. Apply re-hashes the target first and refuses if the
    /// disk changed since the plan. Directories emptied by removal are
    /// pruned.
    Remove {
        /// The hash the manifest records; the target must still match it at
        /// apply time.
...
pub enum Refusal {
    ...
    /// `..`, or containing empty or `.` components; see
