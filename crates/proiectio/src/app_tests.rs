...
/// `status` cannot refuse — `observe` fails with I/O and depth errors alone —
/// so the refusal path is proven over a stand-in handler returning the one
/// error variant carrying a refusal, through the seam every command maps with.
#[handler]
...
/// The merged listing every scopeless `config list` renders here.
...
/// Stock macOS `tar` puts an AppleDouble `._*` sibling at depth 1, which
/// `--strip 1` leaves with no path.
#[test]
#[serial]
...
/// The tour's three entries with the settings file's contents changed.
...
/// No command line reaches an apply-time refusal on its own — deciding and
...
/// The same refusal from a run whose command line carried `--force`.
#[handler]
...
/// stand-in run plans a skip, which the edit turns into a refusal.
...
fn the_csv_a_run_that_could_not_record_writes_says_so_on_stderr() {
    ...
    // Two runs over two destinations: a second run over the first would meet
    // the writes the first one left unrecorded.
...
/// Two owners holding `real/keep.txt` and a `pivot` link, the link deleted
/// by hand so a plan sees no link and writes under the path all the same.
...
/// The warnings a run wrote to stderr.
