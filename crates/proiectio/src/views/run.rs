...
//!
//! One document shape covers three outcomes: a dry run's plan, a completed
//! apply, and a run that stopped part-way. `phase` names the tense, and every
//! arm states its rows at `rows` — every path the pass has a verdict for,
//! refusals included — so a reader branches on `phase` rather than on which
//! keys the document happens to carry, and one CSV projection selects the
//! rows of all three. Verdict vocabularies stay per-tense: a plan says
//! `Write` where a run says `Written`, which is what `phase` tells apart.
//!
//! A stopped run's run-level facts — how far it got, what stopped it, whether
//! the manifest was recorded — are not rows: they render past the counts in
//! the template body, and reach structured callers on stderr via [`warnings`],
//! never in the CSV. Archive members `strip` erased reached no path, so they
//! ride beside the rows (`dropped`) rather than among them.
//!
...
//! Verdicts, refusal kinds, and block faults this CLI does not know render as
//! their own serialized names, escaped, rather than failing: the library may
//! grow vocabulary before this CLI learns the words.
...
impl PlannedRun {
    /// The document for a refusal the library reported as an error. Only a
    /// refusal raised after archive expansion carries `dropped`.
...
/// What a run applied. [`ApplyReport`] nests its rows under `report`; here
/// they flatten, so an applied document states its rows where a planned one
/// does.
#[derive(Serialize)]
...
/// Destructures the library's report whole, so a field the library adds fails
/// to compile here rather than going missing from the documents.
...
/// One row per refused key, stated as a plan's own refused rows would state
/// it, so a refusal reads alike whichever stage reported it.
...
/// One verdict of a run that could not finish: what an action did to the
/// path, or the refusal a declined key met.
#[derive(Serialize)]
#[serde(untagged)]
...
/// verdict for — what it applied, and the keys a refusal declined, as rows of
/// one sequence — and, past the rows, what stopped it.
#[derive(Serialize)]
pub(crate) struct AbortedRun {
    ...
    pub(crate) dropped: BTreeSet<Dropped>,
    pub(crate) manifest: Manifest,
    /// Whether the state directory records the applied rows; false when
    /// writing the manifest failed, so a later run judges the destination
    /// against the record that stood before this one.
    pub(crate) recorded: bool,
    /// The failure a non-refusal stopped with, and the failure to write the
    /// manifest — each stated once. Empty for a refusal that recorded what it
    /// applied, whose refused rows say the whole of it.
...
/// The applied rows and the refused rows as one sequence. The two name
/// disjoint paths — a run acts on nothing it refuses — and a key that reached
/// both stages reads as the refusal, which is what the run left the path at.
fn ran(
...
/// `None` for a refusal, whose refused rows state it in the table instead.
...
/// What `run.jinja` prints.
#[derive(Debug, Default, PartialEq, Eq, Serialize)]
pub(crate) struct RunLines {
    pub(crate) rows: Vec<RowView>,
    /// One line per refusal kind something lifts — once per kind, not per
    /// row: fifty drifted paths are one `--force` away, and saying so fifty
    /// times buries the paths.
    pub(crate) hints: Vec<String>,
    pub(crate) summary: Option<String>,
    pub(crate) stopped: Vec<String>,
}

/// The style, applied verb, and planned verb one verdict reads as; `None`
/// for a verdict this CLI does not know.
fn spelling(verdict: &str, symlink: bool) -> Option<(&'static str, &'static str)> {
    Some(match (verdict, symlink) {
        ...
        // The one verdict a dry run and a real run spell alike: neither did
        // anything.
...
/// A refusal reads in the tense of the pass that met it.
...
/// Why the row overwrites, or why it refuses and which source named it.
...
/// it with. `into` is where an expansion puts an archive, which only a
/// dropped member has to say.
...
/// The refusal's name, in the vocabulary the exit table uses, and what the
/// refusal carries after it.
...
/// What a refusal's payload says beyond its name. `None` where the kind
/// carries nothing, or the payload is not the shape the library serializes.
...
/// What lifts a refusal serialized under `name`; `None` for a kind nothing
/// lifts, or one this CLI does not know. `forced` drops the drift hint: it
/// names `--force`, and this run already carried the flag.
...
/// The sentence `BlockFault`'s own message spells; a fault this CLI does not
/// know reads as its serialized name, escaped.
...
impl Tally {
    ...
    /// Names only the columns that hold something, except that `written` and
    /// `skipped` appear together whenever either does. `not recorded` and
    /// `refused` count apart from all of it: the run did nothing at those
    /// paths, and `nothing to do` over rows naming them would mislead.
...
/// The least each tense's verb column is ever wide; an unknown verdict's own
/// name can widen it. The tests drive every verdict the library declares
/// through `spelling` and check the verb still fits its tense's constant.
...
pub(crate) fn lines(document: &JsonValue, width: AmbiguousWidth, forced: bool) -> RunLines {
    ...
    // An unknown verdict's own name can outgrow the tense's widest verb, so
    // the column takes the widest verb it actually holds.
    let verbs = lines
        .iter()
...
/// The line a real run closes with: its counts, and — where the run could
/// not finish — how far it got.
...
/// Deliberately says nothing per-path: deriving how each path reads to the
/// next run would mean keeping the pre-run manifest to replay the deciding
/// against, which is recovery machinery this CLI does not carry.
...
/// The run-level facts a stopped run states on stderr rather than in the
/// rows: how far it got, what stopped it, and — where the manifest never
/// landed — what the destination is left holding. Empty for every document
/// that is not a stopped run's.
...
/// The `strip` count that left the member with no path, and the archive that
/// carried it — with the place that archive was bound for, which is what
/// tells two entries expanding one archive apart.
...
/// A verdict is a bare name, or a name carrying fields; this reads both.
...
/// under a header that does not move; one projection serves every tense.
/// `detail` is what the verdict variant carries past its name — `Overwrite`
/// its reason, `Refuse` its refusal. The `phase` cell is what reads the
/// verdict cell: the vocabularies are per-tense and both spell `NotRecorded`,
/// so a dry run's record and an applied run's can otherwise be the same
/// bytes.
...
/// A bare name as the name; anything carrying fields as its JSON, because a
/// column cannot hold one shape per variant.
