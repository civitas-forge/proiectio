...
2. Classification and Apply
    ...
    classification against it to choose actions. A non-UTF-8 entry on
    disk can never match a desired or a recorded path, so it stays
    outside the table — never overwritten, never removed, and a
