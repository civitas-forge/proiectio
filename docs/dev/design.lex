...
2. Classification and Apply
    ...
    they run in: a plan is never decided over a replay of its own
    steps. Cleaning up a destination other tools or hands reshaped
    can therefore take more than one run, each deciding over the
    disk the one before left.

    Proiectio has no notion of git: a caller that wants owned
